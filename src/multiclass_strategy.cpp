...
OneVsRestStrategy::OneVsRestStrategy() : library_type_(SVMLibrary::LIBLINEAR) {}

OneVsRestStrategy::~OneVsRestStrategy() {
    cleanup_models();
}

TrainingMetrics OneVsRestStrategy::fit(
...
std::vector<std::vector<double>> OneVsRestStrategy::decision_function(
    const torch::Tensor& X,
    DataConverter& converter) {
    if (!is_trained_) {
        throw std::runtime_error("Model is not trained");
...
double OneVsRestStrategy::train_binary_classifier(
    const torch::Tensor& X,
...
double OneVsRestStrategy::get_sample_decision_value(
...
double OneVsRestStrategy::get_sample_probability(
    const torch::Tensor& sample,
    size_t model_idx,
    DataConverter& converter) const {
    if (library_type_ == SVMLibrary::LIBSVM && svm_models_[model_idx]) {
...
OneVsOneStrategy::OneVsOneStrategy() : library_type_(SVMLibrary::LIBLINEAR) {}

OneVsOneStrategy::~OneVsOneStrategy() {
    cleanup_models();
}

TrainingMetrics OneVsOneStrategy::fit(
    const torch::Tensor& X,
    const torch::Tensor& y,
    const KernelParameters& params,
    DataConverter& converter) {
...
double OneVsOneStrategy::train_pairwise_classifier(
    const torch::Tensor& X,
    const torch::Tensor& y,
    int class1,
    int class2,
